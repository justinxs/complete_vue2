...
module.exports = (app) => {
  function formatOpt(ctx, options) {
    // gw live anchor familys notice members discover resource recharge game vip match signin activity shop sports
    const urlType = options.urlType || 'gw';
    const baseURL = global.appConfig.api[urlType];
    const salt = global.appConfig.salt;
    const timestamp = Math.ceil(Date.now() / 1000);
    const cip = getClientIp(ctx);
    const langCode = ctx.langCode;
    ...
    options.headers = {
      ...(options.headers || {}),
      'x-token': ctx.token || '',
      'x-value': `${serviceId},${ctx.uuid},${timestamp},${ctx.version},${langCode}`,
      signature: md5(`${ctx.uuid}${timestamp}${salt}${ctx.token || ''}`),
      'user-agent': ctx.ua || '',
      'X-Real-IP': cip,
      'x-real-ip': cip
    };
