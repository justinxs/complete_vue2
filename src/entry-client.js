...
import { transformStatusBar } from './initData';
...
function start() {
  ...
  const statusBarHeight = store.state.statusBarHeight;
  const [live_gaming_try, category] = getQueryString([
    'live_gaming_try',
    'category'
  ]);

  store.commit('setStatusBarHeight', transformStatusBar(statusBarHeight));
  store.commit('sports/updateGameTrying', Boolean(+live_gaming_try));
  store.commit('setCategory', category || 'video');
